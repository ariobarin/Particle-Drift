...
@Override
	...
	private void drawLidarData(Graphics g){
		g.setColor(Color.BLACK);
		LidarData data = LiCar.getLidar();
		if (data == null){
			System.out.println("No data");
			return;
		}
		for (int i = 0; i < data.getLidar().length; i++){
			LidarVector vector = data.getLidar()[i];
			double angle = vector.getDirection()*Math.PI/180;
			int distance = vector.getDistance();
			g.drawLine(WIDTH/2, HEIGHT/2, WIDTH/2 + (int)Math.round(distance*(Math.cos(angle))), (int)Math.round(HEIGHT/2 + distance*(Math.sin(angle))));
		}
